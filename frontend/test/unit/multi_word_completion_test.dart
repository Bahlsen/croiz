...
void main() {
  group('Multi-word completion tests', () {
    ...
    test('completing a single horizontal word flashes its cells and plays success once', () {
    ...
    test('completing a single vertical word flashes its cells and plays success once', () {
      final container = ProviderContainer(
        overrides: [
          gameAudioServiceProvider.overrideWithValue(mockAudio),
          flashClearDelayProvider.overrideWithValue(Duration.zero),
      ...
      final board = GameBoard(
        ...
        difficulty: 1,
        entries: const [
          PuzzleEntryData(number: 1, direction: 'down', x: 0, y: 0, length: 3, answer: 'DOG'),
        ],
        solutionGrid: solution,
      );
    ...
    test('completing two words at intersection flashes ALL cells and plays success ONCE', () {
      final container = ProviderContainer(
        overrides: [
          gameAudioServiceProvider.overrideWithValue(mockAudio),
          flashClearDelayProvider.overrideWithValue(Duration.zero),
      ...
      final board = GameBoard(
        ...
        entries: const [
          PuzzleEntryData(number: 1, direction: 'across', x: 0, y: 0, length: 3, answer: 'CAT'),
          PuzzleEntryData(number: 2, direction: 'down', x: 1, y: 0, length: 3, answer: 'ACE'),
        ],
        solutionGrid: solution,
      );

      container.read(gameBoardProvider.notifier).state = board;
      container.read(selectedCellProvider.notifier).value = const SelectedCell(0, 1);
      container.read(wordDirectionProvider.notifier).value = WordDirection.horizontal;
      ...
      fakeAsync((async) {
        ...
        expect(mockAudio.successCount, 1, reason: 'Success sound should play only once for multiple words');

        // ALL cells from BOTH words should be flashing
        final flashing = container.read(flashingCellsProvider);
        expect(flashing.length, 5, reason: 'Should flash 5 unique cells (3 from CAT + 3 from ACE - 1 shared)');
        expect(flashing, containsAll([
        ...
        final found = container.read(foundWordsProvider);
        expect(found.length, 2, reason: 'Both words should be marked as found');

        async.elapse(const Duration(milliseconds: 100));
      });
    });

    test('completing only one word when typing at intersection (other word incomplete)', () {
      final container = ProviderContainer(
        overrides: [
          gameAudioServiceProvider.overrideWithValue(mockAudio),
          flashClearDelayProvider.overrideWithValue(Duration.zero),
    ...
    test('typing a letter that does not complete any word does not trigger flash or sound', () {
      final container = ProviderContainer(
        overrides: [
          gameAudioServiceProvider.overrideWithValue(mockAudio),
          flashClearDelayProvider.overrideWithValue(Duration.zero),
    ...
    test('already found word is skipped when typing at intersection', () {
      ...
      final board = GameBoard(
        ...
        entries: const [
          PuzzleEntryData(number: 1, direction: 'across', x: 0, y: 0, length: 3, answer: 'CAT'),
          PuzzleEntryData(number: 2, direction: 'across', x: 0, y: 1, length: 3, answer: 'DOG'),
        ],
        solutionGrid: solution,
      );
    ...
    test('completing all words triggers victory sound', () {
      ...
      fakeAsync((async) {
        ...
        expect(mockAudio.successCount, 1);
        expect(mockAudio.victoryCount, 1, reason: 'Victory sound should play when all words are found');
    ...
    test('completing three words at once with shared cells', () {
      ...
      final board = GameBoard(
        ...
        difficulty: 1,
        entries: const [
          PuzzleEntryData(number: 1, direction: 'across', x: 0, y: 1, length: 3, answer: 'DEF'),
          PuzzleEntryData(number: 2, direction: 'down', x: 1, y: 0, length: 3, answer: 'BEH'),
        ],
        solutionGrid: solution,
      );
    ...
    test('cells are locked after word completion and cannot be modified', () {
      ...
      final board = GameBoard(
        ...
        entries: const [
          PuzzleEntryData(number: 1, direction: 'across', x: 0, y: 0, length: 3, answer: 'ABC'),
          PuzzleEntryData(number: 2, direction: 'across', x: 0, y: 1, length: 3, answer: 'DEF'),
        ],
        solutionGrid: solution,
      );
    ...
    test('partial word with correct letters does not complete until all filled', () {
      final container = ProviderContainer(
        overrides: [
          gameAudioServiceProvider.overrideWithValue(mockAudio),
          flashClearDelayProvider.overrideWithValue(Duration.zero),
      ...
      final board = GameBoard(
        ...
        difficulty: 1,
        entries: const [
          PuzzleEntryData(number: 1, direction: 'across', x: 0, y: 0, length: 4, answer: 'TEST'),
        ],
        solutionGrid: solution,
      );
