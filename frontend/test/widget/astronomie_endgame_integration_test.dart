...
void main() {
  testWidgets('Completing last prefilled cell in astronomie triggers end overlay', (tester) async {
    final jsonString = File('assets/data/astronomie_puzzle.json').readAsStringSync();
    ...
    var board = PuzzleConverter.puzzleToGameBoard(puzzle, preFillSolutions: true);
    int? clearR, clearC;
    ...
    container.read(selectedCellProvider.notifier).state = SelectedCell(clearR, clearC);
    ...
    if (missingKeys.isNotEmpty) {
      ...
      final edir = m[2];
      final e = entries.firstWhere((en) => en.x == ex && en.y == ey && en.direction == edir);
      final cells = <String?>[];
      final blacks = <bool>[];
      ...
      debugPrint('entry $missingKey cells=$cells blacks=$blacks');
        ...
        final isComplete = svc.isWordComplete(container.read(gameBoardProvider), e);
        debugPrint('svc.isWordComplete($missingKey)=$isComplete');
    }

    // End overlay should be visible
    expect(find.text('Bravo !'), findsOneWidget,
      reason: 'Overlay not shown; found=${found.length} entries=${entries.length}');
  });
}
