...
void main() {
  testWidgets('Reveal one word then revealAll only flashes newly revealed words',
    ...
    await tester.pumpWidget(UncontrolledProviderScope(
      container: container,
      child: const MaterialApp(home: Scaffold(body: Center(child: SizedBox()))),
    ));
    ...
    final entry1 = {const CellKey(0, 0), const CellKey(0, 1), const CellKey(0, 2)};
    ...
    final entry2 = {const CellKey(1, 0), const CellKey(1, 1), const CellKey(1, 2)};
  ...
  testWidgets('Reveal a single letter then revealAll only flashes newly revealed words',
      (WidgetTester tester) async {
    const entries = [
      PuzzleEntryData(number: 1, direction: 'across', x: 0, y: 0, length: 3, answer: 'CAT'),
      PuzzleEntryData(number: 2, direction: 'across', x: 0, y: 1, length: 3, answer: 'DOG'),
